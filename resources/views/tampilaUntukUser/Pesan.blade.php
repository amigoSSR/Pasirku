<x-layout-user title="Pesan" :fullHeight="true">
  ...
  <div class="flex flex-1 overflow-hidden h-full" x-data="chatComponent()" x-init="init()">
    ...
    <section class="bg-surface-container-low flex flex-col h-full overflow-hidden w-[340px] lg:w-[380px] flex-shrink-0 border-r border-outline-variant/30">
      <div class="p-5 border-b border-outline-variant/30">
        <h2 class="font-headline text-xl font-bold text-on-surface mb-4">Conversations</h2>
        <div class="relative">
          ...
          <input class="w-full bg-surface-container-lowest border border-outline-variant/30 rounded-xl pl-9 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary/30 outline-none transition-all"
            placeholder="Filter percakapan..." type="text" x-model="search" />
        </div>
      </div>
      <div class="flex-1 overflow-y-auto space-y-px">
        <template x-for="contact in filteredContacts" :key="contact.id">
          <div @click="selectContact(contact)"
            :class="{'bg-surface-container-lowest border-l-4 border-l-primary': selectedContact && selectedContact.id === contact.id, 'hover:bg-surface-container border-l-4 border-l-transparent': !selectedContact || selectedContact.id !== contact.id}"
            class="px-5 py-4 cursor-pointer transition-colors">
            <div class="flex gap-3">
              <div class="relative flex-shrink-0">
                <div class="w-11 h-11 rounded-xl bg-primary/10 text-primary flex items-center justify-center font-headline font-bold text-lg uppercase" x-text="contact.name.charAt(0)"></div>
                <span class="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 border-2 border-surface-container-lowest rounded-full"></span>
              </div>
              <div class="flex-1 min-w-0">
                <h4 class="font-bold text-sm text-on-surface truncate" x-text="contact.name"></h4>
                <p class="text-xs text-on-surface-variant truncate mt-0.5">Mulai percakapan...</p>
              </div>
            </div>
          </div>
        </template>
        <template x-if="filteredContacts.length === 0">
          <div class="text-center py-12 text-on-surface-variant">
            ...
            <p class="text-sm font-semibold">Tidak ada kontak</p>
          </div>
        </template>
      </div>
    </section>
        ...
        <div class="flex flex-col h-full">
          ...
          <div class="px-6 py-4 flex justify-between items-center bg-surface-container-lowest border-b border-outline-variant/30 shadow-sm z-10">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center font-headline font-bold text-base" x-text="selectedContact.name.charAt(0)"></div>
              <div>
            ...
            <div class="flex items-center gap-2">
              <button class="w-8 h-8 bg-surface-container rounded-xl flex items-center justify-center text-on-surface-variant hover:bg-primary/10 hover:text-primary transition-colors">
      ...
      <template x-if="!selectedContact">
        <div class="flex-1 flex flex-col items-center justify-center text-on-surface-variant h-full">
          <div class="w-20 h-20 bg-surface-container rounded-full flex items-center justify-center mb-4">
            <span class="material-symbols-outlined text-4xl opacity-30">forum</span>
          </div>
          <p class="font-semibold text-sm">Pilih percakapan untuk mulai chat</p>
          <p class="text-xs mt-1 opacity-60">Semua percakapan Anda tersimpan di sini</p>
        </div>
      </template>
          ...
          fetch('{{ route('chat.rooms') }}')
            ...
            .then(data => {
              ...
              if (openChatToko) {
                 ...
                 if (savedId) { const saved = data.find(c => c.id == savedId); if (saved) this.selectContact(saved); }
              }
            });
        },
