    ...
    <div class="bg-surface-container-lowest rounded-2xl shadow-sm border border-outline-variant/20 overflow-hidden">
      <div class="px-6 py-5 border-b border-outline-variant/20 flex items-center justify-between">
        <div>
          ...
          <p class="text-xs text-on-surface-variant mt-0.5">Klik lokasi untuk mengedit alamat & koordinat</p>
        </div>
      </div>
      ...
      <div class="md:hidden divide-y divide-outline-variant/20">
        ...
        <div class="p-5 space-y-3">
          ...
          <div class="grid grid-cols-2 gap-2 text-xs text-on-surface-variant">
            <div onclick='window.dispatchEvent(new CustomEvent("open-store-edit-modal", { detail: { toko: @json($toko) } }))' class="cursor-pointer group/loc">
              <span class="font-semibold text-on-surface">Lokasi:</span> 
              <span class="group-hover/loc:text-primary transition-colors">{{ $toko->Lokasi_Toko }}</span>
              @if($toko->latitude && $toko->longitude)
              <div class="text-[10px] text-on-surface-variant/70 font-mono mt-0.5 flex items-center gap-1">
          ...
          <div class="pt-2 border-t border-outline-variant/10">
            <button type="button" onclick='window.dispatchEvent(new CustomEvent("open-store-edit-modal", { detail: { toko: @json($toko) } }))'
        ...
        <table class="w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-outline-variant/10">
            ...
            <tr class="hover:bg-surface-container-low/40 transition-colors">
              ...
              <td class="px-6 py-4">
                <div onclick='window.dispatchEvent(new CustomEvent("open-store-edit-modal", { detail: { toko: @json($toko) } }))' 
                     class="cursor-pointer hover:text-primary transition-colors group/loc"
                     title="Klik untuk edit lokasi & koordinat">
                  <div class="text-xs text-on-surface-variant max-w-[180px] truncate flex items-center gap-1">
                    <span class="truncate">{{ $toko->Lokasi_Toko ?? '-' }}</span>
                    <span class="material-symbols-outlined text-[14px] opacity-0 group-hover/loc:opacity-100 transition-opacity">edit</span>
                  </div>
                  @if($toko->latitude && $toko->longitude)
                  <p class="text-[10px] text-outline mt-0.5 font-mono flex items-center gap-0.5">
